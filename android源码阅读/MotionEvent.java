...
/**
 * 开放宣言android.view.MotionEvent


对象用于报告运动（鼠标，笔，手指，轨迹球）的事件。运动事件可能持有绝对或相对运动和其他数据，这取决于设备的类型。

概观
运动事件描述的动作代码和一组轴价值观方面的动作。动作代码指定的状态变化发生诸如指针将向下或向上。轴值描述的位置和其他的运动性能。

例如，当用户第一次触摸屏，该系统提供了一个触摸事件与动作码ACTION_DOWN相应视图和一组轴的值，包括X和触摸和左右的压力，尺寸信息的Y
坐标和接触区域的取向。

某些设备可以在同一时间报告多个运动轨迹。多触摸屏发出的每个手指一个动作的痕迹。产生运动轨迹的各个手指或其它物体被称为指针。运动
赛事包括全部是即使他们中的一些根本就没有移动自上次事件被送到当前活动的指针信息。

指针的数量永远只能由一个个别指针上上下下，当手势被取消，除了改变。

每个指针都有一个唯一的ID，当它第一次出现故障分配（按ACTION_DOWN或ACTION_POINTER_DOWN表示）。指针ID一直有效，直到指针最终上升时
或手势被取消（由ACTION_CANCEL表示）（由ACTION_UP或ACTION_POINTER_UP表示）。

的MotionEvent类提供许多方法来查询位置和指针的其他性能，如的getX（INT），的getY（INT），getAxisValue，getPointerId（INT），
getToolType（INT），和许多其他。这些方法大多接受指针索引作为参数，而不是指针标识。在事件的每个指针的指针指数范围比（）
由getPointerCount返回的值小于0至之一。

其中各个指针出现在运动事件中的顺序是不确定的。因此，一个指针的指针索引可以从一个事件到下一但指针的指针标识保证只要指针保持活
性保持恒定不变。使用getPointerId（int）方法来获得一个指针的指针ID跨越的姿势所有后续动作事件跟踪。那么对于连续的动作事件，
使用findPointerIndex（int）方法来获取指针指数在运动事件给定的指针ID。

（）小鼠和触笔按钮可以用getButtonState检索。这是检查按钮状态，而处理ACTION_DOWN作为触摸事件的一部分是一个好主意。该应用程序
可以选择，如果触摸事件开始执行一些不同的操作，由于一个辅助按钮的点击，如呈现的上下文菜单。

配料
为了提高效率，用ACTION_MOVE运动事件可能批量在一起的单个对象中的多个运动样本。最新的指针坐标都可以使用的getX（int）和的getY（INT）。在该批次中较早的坐标采用getHistoricalX（INT，INT）和getHistoricalY（INT，INT）进行访问。坐标是“历史性的”，只是因为他们比该批次中的当前坐标旧的;然而，他们仍然从之前移动事件报告的任何其他坐标不同。要处理该批次按时间顺序所有的坐标，首先消耗历史坐标，然后消耗的当前坐标。

例如：消费在运动事件中的所有指针所有样品按时间顺序。



 无效printSamples（MotionEvent EV）{
     最终诠释historySize = ev.getHistorySize（）;
     最终诠释pointerCount = ev.getPointerCount（）;
     对于（INT H = 0; H <historySize; H ++）{
         System.out.printf（“在时％d：”，ev.getHistoricalEventTime（H））;
         为（中间体p = 0时，P <pointerCount，P ++）{
             System.out.printf（“指针％D：（％F，％F）”，
                 ev.getPointerId（P），ev.getHistoricalX（P，H），ev.getHistoricalY（P，H））;
         }
     }
     System.out.printf（“在时％d：”，ev.getEventTime（））;
     为（中间体p = 0时，P <pointerCount，P ++）{
         System.out.printf（“指针％D：（％F，％F）”，
...
设备类型
的MotionEvent内容的解释有所不同显著取决于源类的设备。

与源类InputDevice.SOURCE_CLASS_POINTER指点设备，诸如触摸屏，鼠标指针的坐标指定绝对位置如图的X / Y坐标。每个完整的手势是
由移动事件与描述指针状态转换和动作的操作序列来表示。手势开始与ACTION_DOWN运动事件，提供了第一台指针式下来的位置。作为这一
下降或达每增加一个指针，该框架将生成ACTION_POINTER_DOWN或ACTION_POINTER_UP相应的运动盛会。指针移动通过与ACTION_MOVE移动事
件描述。最后，一​​个手势或者结束时的最后指针上升通过与ACTION_UP运动事件代表或者手势与ACTION_CANCEL取消。

一些指示设备，如鼠标可以支持垂直和/或水平滚动。 scroll事件被报告为与ACTION_SCROLL通用移动事件，其中包括相对滚动的AXIS_VSCROLL
和AXIS_HSCROLL轴偏移。有关检索这些附加轴信息，请参阅getAxisValue（INT）。

在与源类InputDevice.SOURCE_CLASS_TRACKBALL轨迹球设备，指针坐标指定的相对运动为X / Y的变化量。轨迹球的手势包括通过ACTION_MOVE偶
有ACTION_DOWN或ACTION_UP移动事件穿插移动事件描述运动的顺序按下或释放轨迹球按钮时。

与源类InputDevice.SOURCE_CLASS_JOYSTICK操纵杆装置，所述指针坐标指定操纵杆轴的绝对位置。操纵杆轴的值是归一化到范围-1.0到1.0，其
中0.0对应于中心位置。可使用InputDevice.getMotionRange得到关于一组可用的坐标轴和运动范围的详细信息。一些常见的摇杆轴AXIS_X，
AXIS_Y，AXIS_HAT_X，AXIS_HAT_Y，AXIS_Z和AXIS_RZ。

请参阅的InputDevice有关不同类型的输入设备和来源如何表示指针坐标的更多信息。

一致性保证
运动事件总是传递给视图作为事件的一致流。什么构成一致流取决于设备的类型而变化。对于触摸事件，一致性意味着指针往下走一次，左右移
动作为一个群体再上去一次，或将被取消。

虽然框架试图提供移动事件的一致流的看法，它不能保证它。某些事件可能被丢弃或通过它们从而递送使得不一致事件流之前包​​含在应用程序的
观点进行修改。意见应始终准备处理ACTION_CANCEL，应该容忍的异常情况，如接收到新的ACTION_DOWN没有先收到了现有的手势的ACTION_UP。
 */
